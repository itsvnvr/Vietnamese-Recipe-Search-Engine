...
def get_ingredients(soup):
    # SỬA Ở ĐÂY: Dùng get_text(" ", strip=True) ngay cả trong điều kiện if
...
def scrape_recipe(url):
    ...
    if recipes: 
        # Trường hợp trang có nhiều công thức (ví dụ: "tổng hợp 5 món...")
        for rec in recipes:
            # SỬA Ở ĐÂY: Thêm " " vào get_text
            title = rec.select_one("h2, h3")
            title = title.get_text(" ", strip=True) if title else "(Không có tiêu đề)"
            
            img_tag = rec.select_one("img.lazy")
            image_url = img_tag.get('src') if img_tag else None
            
            video_tag = rec.select_one("iframe[src*='youtube']")
            video_url = video_tag.get('src') if video_tag else None

            # SỬA Ở ĐÂY (từ hàm get_ingredients): Thêm " " vào get_text
            ingredients = [span.get_text(" ", strip=True) for span in rec.select(".staple span") if span.get_text(" ", strip=True)]
        ...
        # Trường hợp trang chỉ có 1 công thức (phổ biến nhất)
        # SỬA Ở ĐÂY: Thêm " " vào get_text
        title = soup.select_one("h1")
        title = title.get_text(" ", strip=True) if title else "(Không có tiêu đề)"
        
        img_tag = soup.select_one("#tongquan > div.video > img")
        image_url = None
        if img_tag:
            image_url = img_tag.get('data-src') or img_tag.get('src')

        video_tag = soup.select_one("div.video-recipe iframe")
        video_url = video_tag.get('src') if video_tag else None
...
def crawl_all(max_pages=50, delay=1):
    ...
    try:
        ...
        print(f"Lỗi nghiêm trọng khi tải trang chính: {e}")
        return # Dừng nếu không tải được trang chính
    ...
    for page in range(2, max_pages + 1):
        try:
            ...
            # Chỉ dừng nếu không có link mới VÀ không có ID mới
            if new == 0 and not new_ids:
                print(f"Trang {page}: Không có link mới. Dừng AJAX.")
                break
            ...
            print(f"Lỗi khi tải trang {page} (AJAX):", e)
            break
    ...
    output_dir = "data/raw"
    output_file = os.path.join(output_dir, "dienmayxanh_raw.json") 
    ...
    print(f"Đã lưu {len(results)} công thức vào {output_file}")
